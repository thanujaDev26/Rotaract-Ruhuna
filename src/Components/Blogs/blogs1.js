...
export default function BlogsOne() {
    ...
    return (
            <div className="p-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {Data.map((data, index) => (
                    <div key={index} className=" h-auto rounded-md bg-white">
                        <div className="w-full h-40 md:h-52 xl:h-64 2xl:h-[400px] bg-no-repeat bg-cover rounded-t-md" style={{backgroundImage:`url(${data.img})`}}>
                        </div>
                        <div className="w-full h-auto text-center p-2">
                            ...
                            <button className="bg-base px-5 py-1 rounded-xl text-white my-2">See more...</button>
                        </div>
                    </div>
                ))}
            </div>
